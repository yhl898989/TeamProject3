...
public class DWUtils {
	...
	public static String itemuploadFile(String uploadPath, String oriName, byte[] fileData) throws Exception {
	...
	public static String makeThumbnail(String uploadPath, String datePath, String newFilename) throws Exception {
		...
		BufferedImage destImg = Scalr.resize(sourceImg, Scalr.Method.AUTOMATIC, // 최소 시간에 가장 잘 보이는 크기 조정 이미지를 얻기 위해 크기
																				// 조정 구현이 사용할 방법을 결정
				Scalr.Mode.FIT_TO_HEIGHT, 100);

		String thumbnamiNamePath = datePath + File.separator + "s_" + newFilename;// 특정 위치에 대한 경로를 구성하는 디렉토리 이름을 구분하는 데
																					// 사용되는 문자
	...
	public static String makeFolder(String parent) {
		...
		return File.separator + year + File.separator + new DecimalFormat("00").format(month) + File.separator
